...
static int max30205_sample_fetch(struct device *dev, enum sensor_channel chan)
{
	...
	if (i2c_burst_read(drv_data->i2c, MAX30205_I2C_ADDR,
			   MAX30205_REG_DATA_START | MAX30205_AUTOINCREMENT_ADDR,
			   buf, 4) < 0) {
...
static int max30205_read_conversion_data(struct max30205_data *drv_data)
{
	u8_t buf[16];

	if (i2c_burst_read(drv_data->i2c, MAX30205_I2C_ADDR,
			   MAX30205_REG_CONVERSION_START |
			   MAX30205_AUTOINCREMENT_ADDR, buf, 16) < 0) {
...
#if CONFIG_MAX30205_TRIGGER
	.trigger_set = max30205_trigger_set,
#endif
...
int max30205_init(struct device *dev)
{
	...
	/* check chip ID */
	if (i2c_reg_read_byte(drv_data->i2c, MAX30205_I2C_ADDR,
			      MAX30205_REG_WHO_AM_I, &id) < 0) {
		SYS_LOG_ERR("Failed to read chip ID.");
		return -EIO;
	}

	if (id != MAX30205_CHIP_ID) {
		SYS_LOG_ERR("Invalid chip ID.");
		return -EINVAL;
	}

	/* check if CONFIG_MAX30205_ODR is valid */
	for (idx = 0; idx < ARRAY_SIZE(max30205_odr_strings); idx++) {
		if (!strcmp(max30205_odr_strings[idx], CONFIG_MAX30205_ODR)) {
			break;
		}
	}

	if (idx == ARRAY_SIZE(max30205_odr_strings)) {
		SYS_LOG_ERR("Invalid ODR value.");
		return -EINVAL;
	}

	if (i2c_reg_write_byte(drv_data->i2c, MAX30205_I2C_ADDR, MAX30205_REG_CTRL1,
			       (idx + 1) << MAX30205_ODR_SHIFT | MAX30205_BDU_BIT |
			       MAX30205_PD_BIT) < 0) {
	...
	if (max30205_read_conversion_data(drv_data) < 0) {
		SYS_LOG_ERR("Failed to read conversion data.");
		return -EINVAL;
	}

#ifdef CONFIG_MAX30205_TRIGGER
	if (max30205_init_interrupt(dev) < 0) {
